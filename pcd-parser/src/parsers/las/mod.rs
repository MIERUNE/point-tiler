...
impl Parser for LasParser {
    fn parse(&self) -> Result<PointCloud, Box<dyn Error>> {
        ...
        let scale_x: f64 = format!("{:.*}", max_digits, 0.1_f64.powi(max_digits as i32)).parse()?;
        let scale_y: f64 = format!("{:.*}", max_digits, 0.1_f64.powi(max_digits as i32)).parse()?;
        let scale_z: f64 = format!("{:.*}", max_digits, 0.1_f64.powi(max_digits as i32)).parse()?;
