...
- **外部ソートによる大規模データ対応** — 保守的なインメモリ見積もりに基づいて、インメモリ処理と外部ソート処理を自動的に切り替えます。RAM に収まるようにファイルを手動で分割する必要がありません。
...
| `--max-memory-mb`      | ワークフロー選択用のメモリ予算（MB 単位）。`推定処理サイズ × 5` がこの値に収まる場合のみ in-memory を選択（デフォルト: 4096） |
...
`--max-memory-mb` は実行時の厳密なメモリ上限ではありません。保守的なワークフロー選択の閾値として使われます。

- `推定処理サイズ × 5 <= max-memory-mb` の場合は in-memory を選択
- それ以外は external sort を選択

この余裕分は、点群バッファ、grouping 用データ構造、並列処理中の中間アロケーションにより、実際の RSS が生の点群サイズを大きく上回るためです。

変換中には一時的な中間ファイルがディスク上に生成されます。現在のベンチマークでは、そのピークサイズは入力 LAS サイズの概ね 1.2 倍から 1.5 倍程度で、処理完了後に自動で削除されます。
...
選択した属性が特定の点に存在しない場合（CSV の空セルや、その項目を持たない LAS の点フォーマットなど）は、省略されるのではなく既定値 `0` として埋め込まれます。
